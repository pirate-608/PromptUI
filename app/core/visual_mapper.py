...
class VisualMapper:
    def __init__(self, config_filename: str = "mappings.json"):
        """
        初始化视觉映射器
        :param config_filename: 配置文件名，默认位于 static 目录下
        """
        ...
        # 2. 尝试加载外部 JSON 配置
        self._load_external_config(config_filename)
    ...
    def _set_defaults(self):
        ...
        self.styles = {
            "清新简洁": "masterpiece, best quality, anime style, soft light, cel shading, 4k, clean lines",
            ...
            "复古风格": "retro artstyle, 90s anime, noise, vintage color palette, lo-fi aesthetic"
        }

    def _load_external_config(self, filename: str):
        """加载外部 JSON 配置文件，支持开发环境和打包环境"""
        try:
            config_path = self._get_resource_path(os.path.join("static", filename))
            
            if not os.path.exists(config_path):
                print(f"[VisualMapper] Warning: Config file not found at {config_path}, using defaults.")
                return
            ...
            print(f"[VisualMapper] Loaded config from {config_path}")
            
        except Exception as e:
            print(f"[VisualMapper] Error loading config: {e}, using defaults.")
